...
std::ofstream csvMetrics;

double droneSpeed = 3.0;
double droneHeight = 50.0;

// Movimiento periódico del dron
void
MoveDrone(Ptr<Node> drone, uint32_t index)
{
    Ptr<MobilityModel> mob = drone->GetObject<MobilityModel>();

    Vector p = mob->GetPosition();

    double speed = droneSpeed;

    //========================================================
    // Trayectoria según el índice del dron
    //========================================================
    if (index % 4 == 0)
    {
        // Movimiento horizontal (+X / -X)
        p.x += speed * direction[index];
    ...
    {
        // Movimiento vertical (+Y / -Y)
        p.y += speed * direction[index];
...
void
WriteMetrics(Ptr<Node> drone,
             Ptr<Node> gnb,
             uint32_t index)
{
    Ptr<MobilityModel> droneMob =
        drone->GetObject<MobilityModel>();

    Ptr<MobilityModel> gnbMob =
        gnb->GetObject<MobilityModel>();

    Vector pd = droneMob->GetPosition();
    Vector pg = gnbMob->GetPosition();

    double distance = CalculateDistance(pd, pg);
    ...
    double freqMHz = 3500.0;

    double distanceKm =
        std::max(distance / 1000.0, 0.001);
...
main(int argc, char* argv[])
{
    ...
    uint16_t gNbNum = 1;
    uint16_t droneNumPergNb = 2; //drones
    bool logging = false;
    bool doubleOperationalBand = true;

    // Traffic parameters (that we will use inside this script):
//ULL para simular control del drone
//BE para multimedia  no importa si hay alguna perdida

    uint32_t udpPacketSizeULL = 100; //ultralow latency flujo pequenio: Bytes
    uint32_t udpPacketSizeBe = 1252; //Paquetes best efford : BYtes
    uint32_t lambdaULL = 10000;  //tasa de paquetes: pkts/s
    uint32_t lambdaBe = 10000;

//BItrate:  R = PacketSIze*lambda

    // Simulation parameters. Please don't use double to indicate seconds; use
    // ns-3 Time values which use integers to avoid portability issues.
    double simTimeSeconds = 3.0;
    //Time simTime = MilliSeconds(3000); //tiempo de simulacion
    Time simTime = Seconds(simTimeSeconds);
    //Time simTime = Seconds(20); //tiempo de simulacion
    Time udpAppStartTime = MilliSeconds(400); //la simulacion empieza despues de
    ...
    uint16_t numerologyBwp1 = 1; //espaciamiento de subportadoras 1: 30kHZ
    double centralFrequencyBand1 = 3.5e9; //frecuencia central
    double bandwidthBand1 = 100e6; //ancho de banda
    uint16_t numerologyBwp2 = 1; //espaciamiento de subportadoras 2
    double centralFrequencyBand2 = 3.5e9; //frecuencia central 2
    double bandwidthBand2 = 100e6;//ancho de banda 2
    double totalTxPower = 35;// potencia totaltx


    CommandLine cmd(__FILE__);

    cmd.AddValue("droneNumPergNb",
             "Number of drones",
             droneNumPergNb);
    ...
    cmd.AddValue("droneNumPergNb", "The number of UE per gNb in multiple-ue topology", droneNumPergNb);
    ...
    gridScenario.SetHorizontalBsDistance(150.0);
    gridScenario.SetVerticalBsDistance(150.0);
    gridScenario.SetBsHeight(25); //altura del gNB
    gridScenario.SetUtHeight(droneHeight); //altura del UE
    // must be set before BS number
    gridScenario.SetSectorization(GridScenarioHelper::SINGLE);
    gridScenario.SetBsNumber(gNbNum);
    gridScenario.SetUtNumber(droneNumPergNb * gNbNum);
    gridScenario.SetScenarioHeight(300); // Create a 300x300 scenario where the UE will
    gridScenario.SetScenarioLength(300); // be distributed.
    randomStream += gridScenario.AssignStreams(randomStream);
    gridScenario.CreateScenario();


    //Inicializar N drones
//    direction.resize(droneNumPergNb);

//    for (uint32_t i = 0; i < droneNumPergNb; i++)
//    {
...
//    for (uint32_t i = 0; i < droneNumPergNb; i++)
//    {
//        Ptr<Node> drone = gridScenario.GetUserTerminals().Get(i);
...
//        double x = 30;
 ...
 //       double y =
//            30 +
//            i * (240.0 / std::max(1u, droneNumPergNb - 1));

//        mob->SetPosition(Vector(x, y, droneHeight));
//    }

    // Inicializar la dirección de cada dron
    //direction.resize(gridScenario.GetUserTerminals().GetN(), 1);

    //NodeContainer drones = gridScenario.GetUserTerminals();

    //drones.Get(0)->GetObject<MobilityModel>()
    //    ->SetPosition(Vector(30.0, 30.0, 50.0));

    //drones.Get(1)->GetObject<MobilityModel>()
    //    ->SetPosition(Vector(270.0, 270.0, 50.0));

    // Mantener el gNB en el centro
    Ptr<Node> gnb = gridScenario.GetBaseStations().Get(0);
    gnb->GetObject<MobilityModel>()->SetPosition(Vector(141.9, 141.9, 25.0)); //(x,y,z)

    Ptr<MobilityModel> mob = gridScenario.GetUserTerminals().Get(0)->GetObject<MobilityModel>();

    std::cout << mob->GetInstanceTypeId().GetName() << std::endl;


    std::cout << "===================================" << std::endl;
    std::cout << "gNBs    : " << gridScenario.GetBaseStations().GetN() << std::endl;
    std::cout << "Drones  : " << gridScenario.GetUserTerminals().GetN() << std::endl;
    std::cout << "===================================" << std::endl;
...
//en esta parte los drones se reparten, para que algunos drones usen ULL y otros Be
    for (uint32_t j = 0; j < gridScenario.GetUserTerminals().GetN(); ++j)
    {
        Ptr<Node> drone = gridScenario.GetUserTerminals().Get(j);
        if (j % 2 == 0)
        {
            ueLowLatContainer.Add(drone);
        }
        else
        {
            ueVoiceContainer.Add(drone);
        }
    }
    ...
    channelHelper->ConfigureFactories("UMi", "Default", "ThreeGpp"); //MOdelo de canal: MIcrocelda Urbano = UMi , Default= LOS/NLOS usa probabilidad, modelo de la Modelo definido por la especificación 3GPP TR 38.901
    /**
     * Use channelHelper API to define the attributes for the channel model (condition, pathloss and
     * spectrum)
     */
    channelHelper->SetChannelConditionModelAttribute("UpdatePeriod", TimeValue(MilliSeconds(0))); //La condicion del canal LOS/NLOS se actualiza simulando movimiento del drone
    channelHelper->SetPathlossAttribute("ShadowingEnabled", BooleanValue(true)); //shadowing
    /*
...
//Caso (i): mismo parámetro para todos los nodos y todas las BWPs. ej numerology, scheduler, beamforming
    ...
    // Core latency
    nrEpcHelper->SetAttribute("S1uLinkDelay", TimeValue(MilliSeconds(0))); //latencia entre core y gNB
...
//NO se usa caso 2 porque comparten misma PHY
...
//SE usa para los distintos anchos de banda
    ...
    AnimationInterface anim("drone-animation.xml");

    anim.SetMobilityPollInterval(MilliSeconds(100));

    anim.SetMaxPktsPerTraceFile(1000000);

    //posiciones generales gnB, drone, remote
    for (uint32_t i = 0; i < NodeList::GetNNodes(); i++)
    {
        ...
        if (!mob)
        {
           // Sólo los nodos sin movilidad
           anim.SetConstantPosition(n, -80, -80);
        }
    ...
    {
       anim.UpdateNodeDescription(gridScenario.GetBaseStations().Get(i), "gNB");
       anim.UpdateNodeColor(gridScenario.GetBaseStations().Get(i),0,0,255);
       anim.UpdateNodeSize(gridScenario.GetBaseStations().Get(i),10,10);
    }
      //drone
    for (uint32_t i = 0; i < gridScenario.GetUserTerminals().GetN(); i++)
    {
       anim.UpdateNodeDescription(gridScenario.GetUserTerminals().Get(i),"Drone");
       anim.UpdateNodeColor(gridScenario.GetUserTerminals().Get(i),255,0,0);
       anim.UpdateNodeSize(gridScenario.GetUserTerminals().Get(i),7,7);
    }


    anim.SetConstantPosition(remoteHost, -40, 0);
    ...
    nrHelper->AttachToClosestGnb(ueLowLatNetDev, gnbNetDev); //Equivalente al registro en la red 
    nrHelper->AttachToClosestGnb(ueVoiceNetDev, gnbNetDev); //EStos attach, el UE busca el gNB mas cercano, UE drones
    ...
    //QOS = Bearer. Identifica el trafico. Bearer asignado a cada flujo
    ...
    dlClientVoice.SetAttribute("PacketSize", UintegerValue(udpPacketSizeBe)); //tamanio de paquetes
    dlClientVoice.SetAttribute("Interval", TimeValue(Seconds(1.0 / lambdaBe))); //cada cuanto se envia

    // The bearer that will carry voice traffic //concepto de 5G bearers: relgas distintas para cada flujo. Canal logico QOS
    ...
    for (uint32_t i = 0; i < ueLowLatContainer.GetN(); ++i)
    {
        ...
        nrHelper->ActivateDedicatedEpsBearer(ueDevice, lowLatBearer, lowLatTft); //asocia el bearer de ULL con un drone
    }
    ...
    for (uint32_t i = 0; i < ueVoiceContainer.GetN(); ++i)
    {
        ...
        // Activate a dedicated bearer for the traffic type
        nrHelper->ActivateDedicatedEpsBearer(ueDevice, voiceBearer, voiceTft); //asocia el bearer de voice con un drone
    }
    ...
    //=====================================================
    // Iniciar movimiento de los drones
    //=====================================================

    //Simulator::Schedule(Seconds(1.0), &MoveDrone, gridScenario.GetUserTerminals().Get(0));

    //Simulator::Schedule(Seconds(1.0), &MoveDrone, gridScenario.GetUserTerminals().Get(1));


    csvFile.open("drone_positions.csv");
    ...
    csvMetrics.open("drone_metrics.csv");
    ...
    uint32_t N = gridScenario.GetUserTerminals().GetN();
    ...
    for (uint32_t i = 0; i < gridScenario.GetUserTerminals().GetN(); i++)
    {
        Simulator::Schedule(
            Seconds(0.1),
            &MoveDrone,
            gridScenario.GetUserTerminals().Get(i),
            i);
    ...
    */
    ...
    else if (argc == 1 and droneNumPergNb == 9) // called from examples-to-run.py with these parameters
    {
