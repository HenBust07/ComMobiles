...
#_-_-_- Perfil envolvente de retardo normalizado mediante la primera potencia del trayecto de llegada
#_-_-_- Cuando la estación base está en el lado izquierdo o derecho de la calle
def simular_fading(params):
    ...
    #cHc = 10 #m, entre 5-50m altura de la estacion movil. Si es 20 cambiar cRc a 0.3 y gmm a -15
    cHc = params["cHc"]
    #hb = 100 #m, entre 5 y 150m altura sobre el nivel del suelo de la estacion movil
    hb = params["hb"]
    #d = 1 #km, entre 0.5 y 3Km en entorno  NLOS. Distancia desde la estación base
    d = params["d"]
    ...
    f = params["f"]
    #cRc = 0.4 #Coeficiente de reflexión de potencia media de los muros laterales del edificio (<1), entre 0.1 y 0.5
    cRc = params["cRc"]
    ...
    #parametros para RICIAN
    gmm_dB = -14 #constante entre -12 a -16dB
    gmm = 10**(gmm_dB/10)
    PDP_los = (cRc)*((np.sqrt(1+(8*(1000*d)*(300*Tau_B)/(W**2)))-1)/2) + gmm*PDPnLoS
    PDP_los_dB = 10 * np.log10(PDP_los)
    #PDP_los = PDP_los / np.sum(PDP_los)
    #graficar perfil de retardo, talvez en funcion Tau
    # ----- GRAFICA PDP
    #plt.figure()
    #plt.stem(Tau_B*1e6, PDP_los_dB)
    #plt.xlabel('Retardo τ (μs)')
    #plt.ylabel('Potencia [dB]')
    #plt.title('Perfil de Retardo ITU LoS Rician')
    #plt.grid()

    # -------- CANAL: H(f) --------

    # Coeficientes
    alpha = np.sqrt(PDP_los) #unid lineales

    # Fase aleatoria (Rician)
    #phi = np.random.uniform(0, 2*np.pi, N)
    phi = np.random.uniform(0, 2*np.pi, N)
    phi[0] = 0  # componente LoS dominante
    ...
    #plt.ylabel('|H(f)| [dB]')
    #plt.title('Respuesta en Frecuencia del Canal Rician')
    #plt.grid()
    #plt.show()
...
def simular_fading_T(params):
    ...
    #N2 = 10#fijo
    N2 = params["N2"]
    #v = 80  # m/s  (ejemplo: ~72 km/h)
    v = params["v"]
    i2 = np.arange(0, N2) # Numero de copias para N fijo
    #fd_max = 500  # Hz (slow fading)
    ...
    gmm_dB = -14 #constante entre -12 a -16dB
    gmm = 10**(gmm_dB/10)
    PDP_los2 = (cRc)*((np.sqrt(1+(8*(1000*d)*(300*Tau_B2)/(W**2)))-1)/2) + gmm*PDPnLoS2
    PDP_los2_dB = 10 * np.log10(PDP_los2)


    #graficar perfil de retardo, talvez en funcion Tau
    # ----- GRAFICA PDP
    #plt.figure()
    #plt.stem(Tau_B2*1e6, PDP_los2_dB)
    #plt.xlabel('Retardo τ (μs)')
    #plt.ylabel('Potencia [dB]')
    #plt.title('Perfil de Retardo ITU LoS Rician')
    #plt.grid()
    ...
    #plt.xlabel('Tiempo (s)')
    #plt.ylabel('|h(t)| [dB]')
    #plt.title('Fading del Canal Rician')
    #plt.grid()
    #plt.show()
